...
namespace MatchGems.View
{
    ...
    public class BoardView : MonoBehaviour
    {
        ...
        public async Task AnimateBulidAsync(BoardModel boardModel, GridMapper gridMapper, float duration)
        {
            _gridMapper = gridMapper;
            //建立移動的清單
            ...
            for(int y = 0; y < boardModel.Height; y++)
            {
                for (int x = 0; x < boardModel.Height; x++)
                {
                    ...
                    if (!boardModel.HasGem(coord)) continue;//無寶石可操作跳過
                    //新建立
                    GemTile tile = CreateTailAt(SpawnAbove(board, coord));
        ...
        private GemTile CreateTile(int x, int y)
        {
            ...
            return CreateTailAt(_gridMapper.ToWorld(new CellCoord(x, y)));
        }

        private GemTile CreateTile(CellCoord coord)
        {
            //Vector3 position = new Vector3(x * CellWorldSize, y * CellWorldSize, 0);
            return CreateTailAt(_gridMapper.ToWorld(new CellCoord()));
        }
        /// <summary>
        /// 建立寶石磚在世界座標
        /// </summary>
        ...
        /// <returns>寶石磚</returns>
        private GemTile CreateTailAt(Vector3 pos)
        {
